...
export async function POST(req: Request) {
  try {
    const { tokenSymbol, owner, network } = await req.json();
    if (!tokenSymbol || !owner || !network) {
      return NextResponse.json(
        { error: "Missing parameters: tokenSymbol, owner, network required" },
        { status: 400 }
      );
    }
    ...
    if (!tokenAddress) {
      return NextResponse.json(
        { error: `Token ${tokenSymbol} not supported on ${network}` },
        { status: 400 }
      );
    }
